...
{
    private static string JsStr(string key) => LanguageManager.GetString(key).Replace("'", "\\'");

    public static string GetEnterCodePage()
    {
        var title = LanguageManager.GetString("HtmlTitle");
        var enterCode = LanguageManager.GetString("HtmlEnterCode");
        var connect = LanguageManager.GetString("HtmlConnect");
...
<body>
    ...
    public static string GetMainPage(string code)
    {
        var title = LanguageManager.GetString("HtmlTitle");
        var uploadToPeer = LanguageManager.GetString("HtmlUploadToPeer");
        var clickOrDrag = LanguageManager.GetString("HtmlClickOrDrag");
        var preparing = JsStr("HtmlPreparing");
        var noShared = JsStr("HtmlNoSharedFiles");
        var uploadSuccess = JsStr("HtmlUploadSuccess");
        var uploadFailed = JsStr("HtmlUploadFailed");
        var networkError = JsStr("HtmlNetworkError");
        var download = JsStr("HtmlDownload");
        var downloadableFiles = LanguageManager.GetString("HtmlDownloadableFiles");
...
<body>
    <div class=""container"">
        ...
        <div class=""card"">
            ...
            <div class=""progress"" id=""progress"">
                ...
                <div class=""text"" id=""progressText"">{preparing}</div>
            </div>
            <div class=""msg"" id=""msg""></div>
        </div>

        <div class=""card"">
            <h2>{downloadableFiles}</h2>
            <ul class=""file-list"" id=""fileList"">
                <li style=""color:#999;text-align:center;display:block"">{noShared}</li>
            </ul>
        </div>
    </div>
    ...
    <script>
        ...
        function uploadFile(file) {{
            ...
            xhr.addEventListener('load', function() {{
                if (xhr.status === 200) {{
                    ...
                    msg.textContent = '{uploadSuccess}: ' + file.name;
                    loadFileList();
                }} else {{
                    msg.className = 'msg error';
                    msg.textContent = '{uploadFailed} (' + xhr.status + '): ' + xhr.responseText;
                }}
        ...
        function updateFileList(files) {{
            ...
            if (!files.length) {{
                list.innerHTML = '<li style=""color:#999;text-align:center;display:block"">{noShared}</li>';
                return;
            }}
            var html = '';
            for (var i = 0; i < files.length; i++) {{
                var f = files[i];
                html += '<li><span class=""file-name"">' + escapeHtml(f.name) + '</span><span class=""file-meta"">' + f.size + ' <a class=""btn"" href=""/download/' + f.id + '?code=' + code + '"" style=""margin-left:8px"">{download}</a></span></li>';
            }}
            list.innerHTML = html;
        }}
    ...
    {
        var errorTitle = LanguageManager.GetString("HtmlErrorTitle");
        var backHome = LanguageManager.GetString("HtmlBackHome");
