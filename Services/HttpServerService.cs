...
public class HttpServerService : IDisposable
{
    ...
    public async Task StartAsync(string ip, int port)
    {
        ...
        _listener.Start();
        OnLog?.Invoke(string.Format(LanguageManager.GetString("ServerStarted"), $"http://{ip}:{port}/"));
    ...
    {
        if (string.IsNullOrEmpty(userAgent)) return LanguageManager.GetString("DeviceUnknown");

        if (userAgent.Contains("iPhone")) return "iPhone";
        if (userAgent.Contains("iPad")) return "iPad";
        if (userAgent.Contains("Android") && userAgent.Contains("Mobile")) return LanguageManager.GetString("DeviceAndroidPhone");
        if (userAgent.Contains("Android")) return LanguageManager.GetString("DeviceAndroidTablet");
        ...
        return LanguageManager.GetString("DeviceUnknown");
    }
            ...
            try
            {
                if (path == "/" && method == "GET")
                {
                    if (string.IsNullOrEmpty(code))
                        await SendHtmlAsync(stream, HtmlTemplates.GetEnterCodePage());
                    else if (code != _connectionCode)
                        await SendHtmlAsync(stream, HtmlTemplates.GetErrorPage(LanguageManager.GetString("InvalidCode")), "403 Forbidden");
                    else
                        await SendHtmlAsync(stream, HtmlTemplates.GetMainPage(_connectionCode));
                }
                else if (code != _connectionCode)
                {
                    await SendJsonAsync(stream, $"{{\"error\":\"{LanguageManager.GetString("InvalidCode")}\"}}", "403 Forbidden");
                }
                ...
                {
                    await SendHtmlAsync(stream, HtmlTemplates.GetErrorPage(LanguageManager.GetString("PageNotFound")), "404 Not Found");
                }
            }
            catch (Exception ex)
            {
                OnLog?.Invoke(string.Format(LanguageManager.GetString("RequestError"), $"{ex.Message}\n{ex.StackTrace}"));
                try
                {
    ...
    private async Task HandleDownloadAsync(TcpClient client, NetworkStream stream, string fileId, string remoteIp, string? rangeHeader)
    {
        ...
        {
            await SendHtmlAsync(stream, HtmlTemplates.GetErrorPage(LanguageManager.GetString("FileExpired")), "404 Not Found");
            return;
        }

        if (!File.Exists(file.FilePath))
        {
            await SendHtmlAsync(stream, HtmlTemplates.GetErrorPage(LanguageManager.GetString("SourceFileDeleted")), "404 Not Found");
            return;
        }
        ...
        if (isRangeRequest)
        {
            ...
            {
                await SendJsonAsync(stream, $"{{\"error\":\"{LanguageManager.GetString("RangeOutOfBounds")}\"}}", "416 Range Not Satisfiable");
                return;
            }

            var contentLen = rangeEnd - rangeStart + 1;
            OnLog?.Invoke(string.Format(LanguageManager.GetString("ResumeTransfer"), file.FileName, FormatSize(contentLen), rangeStart, rangeEnd, realSize));
            ...
            OnLog?.Invoke(string.Format(LanguageManager.GetString("ResumeComplete"), file.FileName));
        }
        else
        {
            OnLog?.Invoke(string.Format(LanguageManager.GetString("DownloadStarted"), file.FileName, FormatSize(realSize), remoteIp));
            ...
            OnLog?.Invoke(string.Format(LanguageManager.GetString("DownloadComplete"), file.FileName, totalSent, totalSent == realSize));
        }
    ...
    {
        OnLog?.Invoke(string.Format(LanguageManager.GetString("UploadRequest"), remoteIp, contentLength, contentType));

        if (contentLength == 0 || string.IsNullOrEmpty(contentType))
        {
            await SendJsonAsync(responseStream, $"{{\"error\":\"{LanguageManager.GetString("NoFileData")}\"}}", "400 Bad Request");
            return;
        }

        if (!contentType.Contains("multipart/form-data"))
        {
            await SendJsonAsync(responseStream, $"{{\"error\":\"{LanguageManager.GetString("UseMultipartFormat")}\"}}", "400 Bad Request");
            return;
        }
        ...
        {
            await SendJsonAsync(responseStream, $"{{\"error\":\"{LanguageManager.GetString("CannotParseBoundary")}\"}}", "400 Bad Request");
            return;
        }
        ...
        OnLog?.Invoke(string.Format(LanguageManager.GetString("ParsingUpload"), boundary));

        var section = await multipartReader.ReadNextSectionAsync();
        if (section == null)
        {
            OnLog?.Invoke(LanguageManager.GetString("NoSectionParsed"));
            await SendJsonAsync(responseStream, $"{{\"error\":\"{LanguageManager.GetString("NoFileDataParsed")}\"}}", "400 Bad Request");
            return;
        }
        while (section != null)
        {
            if (section.ContentDisposition?.Contains("filename") == true)
            {
                ...
                OnLog?.Invoke(string.Format(LanguageManager.GetString("ReceiveStarted"), fileName, remoteIp, contentLength));
                ...
                while ((bytesRead = await section.Body.ReadAsync(buffer)) > 0)
                {
                    ...
                    if (readCount <= 3 || readCount % 100 == 0)
                        DebugLog.Write($"Upload read #{readCount}: {bytesRead} bytes, total {totalSize} bytes");
                }
                DebugLog.Write($"Upload complete: {fileName}, reads={readCount}, totalSize={totalSize} bytes, Content-Length={contentLength}");
                ...
                OnLog?.Invoke(string.Format(LanguageManager.GetString("ReceiveComplete"), finalName, FormatSize(totalSize)));
        ...
        await SendJsonAsync(responseStream, $"{{\"error\":\"{LanguageManager.GetString("FileNotFoundInRequest")}\"}}", "400 Bad Request");
    }
    ...
    private async Task HandleSseAsync(TcpClient client, NetworkStream stream)
    {
        ...
        OnLog?.Invoke(string.Format(LanguageManager.GetString("SseConnected"), clientId));
        ...
        {
            OnDevicesChanged?.Invoke(GetConnectedDevices());
            OnLog?.Invoke(string.Format(LanguageManager.GetString("DeviceDisconnected"), remoteIp));
        }
    }
